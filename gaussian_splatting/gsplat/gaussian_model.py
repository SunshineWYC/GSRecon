...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_unit_scale=20.0):
        self.spatial_lr_scale = spatial_unit_scale
        points_xyz = torch.tensor(np.asarray(pcd.points), dtype=torch.float32, device=self.device)
        points_color = torch.tensor(np.asarray(pcd.colors), dtype=torch.float32, device=self.device)

        dist2 = torch.clamp_min(distCUDA2(points_xyz), 0.0000001)
        # filter points scale is too large
        valid_mask = dist2 < torch.quantile(dist2, 0.99)
        points_xyz = points_xyz[valid_mask]
        points_color = points_color[valid_mask]
        dist2 = dist2[valid_mask]

        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)

        fused_point_cloud = points_xyz
        fused_color = RGB2SH(points_color)
        ...
        opacities = self.inverse_opacity_activation(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float32, device=self.device))
    ...
    def training_setup(self, training_args):
        ...
        if self.optimizer_type == "default":
            ...
            try:
                self.optimizer = torch.optim.SparseAdam(l, lr=0.0, eps=1e-15)
            except:
            ...
            try:
                ...
                self.optimizer = SelectiveAdam(l, lr=0.0, eps=1e-15, betas=(0.9, 0.999))
            except:
                # A special version of the rasterizer is required to enable sparse adam
                self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
    ...
    def load_ply(self, path, use_train_test_exp = False):
        ...
        if use_train_test_exp:
            ...
            if os.path.exists(exposure_file):
                ...
                self.pretrained_exposures = {image_name: torch.tensor(exposures[image_name], dtype=torch.float32, device=self.device).requires_grad_(False) for image_name in exposures}
        ...
        self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float32, device=self.device).requires_grad_(True))
        self._features_dc = nn.Parameter(torch.tensor(features_dc, dtype=torch.float32, device=self.device).transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(torch.tensor(features_extra, dtype=torch.float32, device=self.device).transpose(1, 2).contiguous().requires_grad_(True))
        self._opacity = nn.Parameter(torch.tensor(opacities, dtype=torch.float32, device=self.device).requires_grad_(True))
        self._scaling = nn.Parameter(torch.tensor(scales, dtype=torch.float32, device=self.device).requires_grad_(True))
        self._rotation = nn.Parameter(torch.tensor(rots, dtype=torch.float32, device=self.device).requires_grad_(True))
    ...
    def densify_and_split(self, grads, grad_threshold, spatial_unit_scale, N=2):
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling, dim=1).values > self.percent_dense*spatial_unit_scale)
    ...
    def densify_and_clone(self, grads, grad_threshold, spatial_unit_scale):
        # Extract points that satisfy the gradient condition
    ...
    def densify_and_prune(self, max_grad, min_opacity, spatial_unit_scale, max_screen_size):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.densify_and_clone(grads, max_grad, spatial_unit_scale)
        self.densify_and_split(grads, max_grad, spatial_unit_scale)

        prune_mask = (self.get_opacity < min_opacity).squeeze()
        if max_screen_size:
            big_points_vs = self.max_radii2D > max_screen_size
            big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * spatial_unit_scale
